...
clinics <- st_read("Basic Needs/02_Data/WIC Clinics/WIC Clinics.gdb", layer = "WIC_Clinics") %>%
  ...
  rename(x21_jul = x7_21_2021,
         x21_aug = x8_21_2021)

sumOld <- sum(clinics$x20_aug, na.rm = TRUE)
sumNew <- sum(clinics$x21_aug, na.rm = TRUE)
...
sumRecent <- sum(clinics$x21_may, na.rm = TRUE)

clinics <- clinics %>%
  mutate(perpart = round((x21_aug/sumNew)*100, digits = 2),
         perchange = round((x21_aug-x20_aug)/sumOld*100, digits = 2),
         permonth = round((x21_aug-x21_jul)/sumOld*100, digits = 2))
```
...
WIClocal <- clinics %>%
  ...
  labs(title = "June 2021 Participation by Agency",
...
as.data.frame(clinics) %>%
  select(site_name, x21_aug, perpart, perchange) %>%
  drop_na(x21_aug) %>%
  kable(caption = "WIC Clinic Participation",
        col.names = c("",
                      "August 2021 Participation",
                      "% of LA7 Participats",
                      "Change Prior Year")) %>%
...
WICurban <- WIC_Participation %>%
  ...
  geom_line() +
  scale_color_manual(values = c("#008097", "#ec008c", "#eaca2d", "#fdddd7", "#b4b4b4", "#9bd9e3", "#996fb1", "#FF7F11")) +
  scale_size_manual(values = c(0.5, 1, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5)) +
  labs(title = "Monthly WIC Participation in Urban Texas Counties",
       subtitle = "Between October 2015 through May 2021",
       x = "",
