...
st_layers("D:/CPAL Dropbox/Basic Needs/04_Projects/WIC/WIC Eligibility/GIS/WIC_EligibilityEstimates.gpkg")

wic_eligible_sf <- st_read("D:/CPAL Dropbox/Basic Needs/02_Data/WIC Eligibility Estimates/WIC_Eligible_2018.shp") %>%
...
```{r, include=FALSE}
setwd("D:/CPAL Dropbox/") #Michael Desktop Directory
...
sumOld <- sum(clinics$x20_jun, na.rm = TRUE)
sumNew <- sum(clinics$x21_jun, na.rm = TRUE)
...
clinics <- clinics %>%
  mutate(perpart = round((x21_jun/sumNew)*100, digits = 2),
         perchange = round((x21_jun-x20_jun)/sumOld*100, digits = 2),
         permonth = round((x21_jun-x21_may)/sumOld*100, digits = 2))
```
...
as.data.frame(clinics) %>%
  select(site_name, x21_jun, perpart, perchange) %>%
  drop_na(x21_jun) %>%
