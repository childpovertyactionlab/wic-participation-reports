...
clinicbuffer <- clinics %>%
  ...
  summarise(WIC_eligible = sum(elig_185*PerIntSm, na.rm = TRUE)) %>%
  ungroup() %>%
  st_transform(crs = 4326) %>%
  mutate(ClinicNo = as.character(ClinicNo)) %>%
...
clinicjoin <- left_join(clinics, clinicbuffer)
...
bins <- BAMMtools::getJenksBreaks(wic_eligDallas_sf$elig_185, k = 6)
pal <- colorBin("BuPu", domain = wic_eligDallas_sf$elig_185, bins = bins)
