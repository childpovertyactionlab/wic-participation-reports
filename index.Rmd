...
clinics <- st_read("Basic Needs/02_Data/WIC Clinics/WIC Clinics.gdb", layer = "WIC_Clinics") %>%
  ...
  mutate(x21_oct = as.numeric(str_remove(x21_oct, ",")),
         x21_nov = as.numeric(str_remove(x21_nov, ",")))

sumOld <- sum(clinics$x20_nov, na.rm = TRUE)
sumNew <- sum(clinics$x21_nov, na.rm = TRUE)
...
sumRecent <- sum(clinics$x21_nov, na.rm = TRUE)

clinics <- clinics %>%
  mutate(perpart = round((x21_nov/sumNew)*100, digits = 2),
         perchange = round((x21_nov-x20_nov)/sumOld*100, digits = 2),
         permonth = round((x21_nov-x21_oct)/sumOld*100, digits = 2))
```
...
#str(clinics)
WIClocal <- clinics %>%
  mutate(x21_apr = as.integer(x21_apr),
         x21_may = as.integer(x21_may),
         x21_jul = as.integer(x21_jul),
         x21_aug = as.integer(x21_aug),
         x21_sep = as.integer(x21_sep),
         x21_oct = as.integer(x21_oct),
         x21_nov = as.integer(x21_nov)
         ) %>%
...
clinics %>%
  ...
  reactable(., columns = list(
    site_name = colDef(
      name = "Clinic"),
    x21_sep = colDef(
      name = "November 2021 Participation"),
    perpart = colDef(
