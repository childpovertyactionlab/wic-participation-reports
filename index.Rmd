...
#wic_eligible_sf <- st_read("C:/Users/micha/CPAL Dropbox/Basic Needs/02_Data/WIC Eligibility Estimates/WIC_Eligible_2018.shp") %>%
...
clinics <- st_read("Basic Needs/02_Data/WIC Clinics/WIC Clinics.gdb", layer = "WIC_Clinics") %>%
  ...
  select(-(october_01_2017:december_01_2019)) %>%
  mutate(november_01_2021 = as.numeric(str_remove(november_01_2021, ",")),
         december_01_2021 = as.numeric(str_remove(december_01_2021, ",")))

sumOld <- sum(clinics$december_01_2020, na.rm = TRUE)
sumNew <- sum(clinics$december_01_2021, na.rm = TRUE)
...
sumRecent <- sum(clinics$december_01_2021, na.rm = TRUE)

clinics <- clinics %>%
  mutate(perpart = round((december_01_2021/sumNew)*100, digits = 2),
         perchange = round((december_01_2021-december_01_2020)/sumOld*100, digits = 2),
         permonth = round((december_01_2021-november_01_2021)/sumOld*100, digits = 2))
```
...
str(clinics)
WIClocal <- clinics %>%
  mutate(november_01_2021  = as.integer(november_01_2021 ),
         december_01_2021  = as.integer(december_01_2021)
         ) %>%
  pivot_longer(cols = january_01_2020:december_01_2021, names_to = "Date", values_to = "Participation") %>%
  ...
  labs(title = "November 2021 Participation by Agency",
...
clinics %>%
  st_drop_geometry(.) %>%
  select(site_name, december_01_2021, perpart, perchange, permonth) %>%
  mutate(perpart = paste0(perpart, "%"),
         perchange = paste0(perchange, "%"),
         permonth = paste0(permonth, "%")) %>%
  drop_na(december_01_2021) %>%
  reactable(., columns = list(
    site_name = colDef(
      name = "Clinic"),
    december_01_2021 = colDef(
      name = "December 2021 Participation"),
    perpart = colDef(
